...
.EXAMPLE
    .\setup-vendure-master.ps1

.EXAMPLE
    .\setup-vendure-master.ps1 -DryRun
...
Write-Host ""
Write-Host "🚀 Entrepreneur OS - Vendure Master Setup" -ForegroundColor Cyan
...
Write-ColorOutput -Message "Verificando base de datos Master..." -Type 'Info'
try {
    $dbCheck = docker exec entrepreneur-postgres-master pg_isready -U vendure 2>&1
    if ($dbCheck -match "accepting connections") {
        Write-ColorOutput -Message "Base de datos Master lista" -Type 'Success'
    } else {
        throw "Database not ready"
    }
} catch {
    Write-ColorOutput -Message "La base de datos Master no está lista." -Type 'Error'
    Write-ColorOutput -Message "Ejecuta: docker-compose up -d postgres-master" -Type 'Info'
...
Write-Host "🎉 ¡Vendure Master configurado exitosamente!" -ForegroundColor Green
Write-Host "================================================" -ForegroundColor Green
Write-Host ""
...
Write-Host "   nx serve vendure-master"
Write-Host ""
Write-Host "3. Configurar Vendure Ecommerce (después de verificar Master):"
Write-Host "   .\tools\scripts\setup-vendure-ecommerce.ps1"
Write-Host ""
Write-Host "URLs de acceso (Vendure Master):" -ForegroundColor Cyan
Write-Host ""
