...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --help|-h)
            ...
            echo ""
            echo "Options:"
            echo "  --dry-run, -n    Run in dry-run mode (print commands without executing)"
...
if [ "$DRY_RUN" -eq 1 ]; then
    echo -e "${MAGENTA}🔍 DRY-RUN MODE ENABLED - No changes will be made${NC}"
    echo ""
fi
...
print_dryrun() {
    ...
    if [ -n "$2" ]; then
        echo -e "${GRAY}  Command: $2${NC}"
    fi
}

# Execute command wrapper (dry-run aware)
execute_cmd() {
    local description="$1"
    shift
    local command="$*"
    ...
    if [ "$DRY_RUN" -eq 1 ]; then
        print_dryrun "$description" "$command"
    else
...
# Create file wrapper (dry-run aware)
create_file() {
    local filepath="$1"
    local content="$2"

    if [ "$DRY_RUN" -eq 1 ]; then
        print_dryrun "Would create file: $filepath"
        echo -e "${GRAY}  Content preview: ${content:0:100}...${NC}"
    else
        echo "$content" > "$filepath"
    fi
}

# Create directory wrapper (dry-run aware)
create_dir() {
    local dirpath="$1"

    if [ "$DRY_RUN" -eq 1 ]; then
        print_dryrun "Would create directory: $dirpath"
    else
        mkdir -p "$dirpath"
    fi
}
...
setup_vendure() {
    ...
    create_dir "apps/$APP_NAME/src"
    create_dir "apps/$APP_NAME/static-assets"
    create_dir "apps/$APP_NAME/static-email-templates"
    create_dir "apps/$APP_NAME/static-email-output"
    create_dir "apps/$APP_NAME/migrations"

    # Crear package.json
    if [ "$DRY_RUN" -eq 1 ]; then
        print_dryrun "Would create package.json in apps/$APP_NAME/"
    else
...
EOF
    ...
    print_info "Instalando dependencias (esto puede tardar 5-10 minutos)..."
...
# Verificar que la base de datos MASTER está disponible
print_info "Verificando base de datos Master..."
MASTER_DB_READY=$(docker exec entrepreneur-postgres-master pg_isready -U vendure 2>&1 | grep "accepting connections" || echo "")

if [ -z "$MASTER_DB_READY" ]; then
    print_error "La base de datos Master no está lista."
    print_info "Ejecuta: docker-compose up -d postgres-master"
    print_info "O consulta: docs/guides/VENDURE-SETUP.md"
    exit 1
fi
...
echo ""
echo "================================================"
echo -e "${GREEN}🎉 ¡Vendure Master configurado exitosamente!${NC}"
echo "================================================"
echo ""
...
echo ""
echo "2. O usar Nx (desde la raíz):"
echo "   nx serve vendure-master"
echo ""
echo "3. Configurar Vendure Ecommerce (después de verificar Master):"
echo "   pnpm run setup:vendure:ecommerce"
echo ""
echo -e "${BLUE}URLs de acceso (Vendure Master):${NC}"
echo ""
...
echo ""
echo "Para más información consulta: docs/guides/VENDURE-SETUP.md"
echo ""
